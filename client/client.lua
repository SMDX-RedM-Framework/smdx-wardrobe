...
RegisterNetEvent('SMDXCore:Client:OnPlayerLoaded')
AddEventHandler('SMDXCore:Client:OnPlayerLoaded', function()
    Wait(3000)

    SMDXCore.Functions.TriggerCallback('smdx-wardrobe:server:getPlayerSkin', function(result)
        local ped = PlayerPedId()
        local male = IsPedMale(ped)

        if male then
            local bodyComponents = exports['smdx-appearance']:GetBodyComponents()
...
RegisterNetEvent('smdx-wardrobe:client:OnOffClothing', function(clothingName)
        ...
        if clothingName == name then
            if name == 'pants' then
                ...
                if clothingData['currentBoots'] == 0 then
                    ClothesCache = exports['smdx-appearance']:GetClothesCache()
...
-- Remove all clothing
RegisterNetEvent('smdx-wardrobe:client:removeAllClothing', function()
    local ped = PlayerPedId()
    local male = IsPedMale(ped)

    TriggerServerEvent('smdx-weapons:server:RemoveAllObjects')
